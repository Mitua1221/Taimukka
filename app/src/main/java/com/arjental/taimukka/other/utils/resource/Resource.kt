...
sealed class Resource<T> {
    data class Loading<T>(val data: T, val percents: Int?)
    data class Error<T>(val data: T, val cause: Throwable)
    data class Success<T>(val data: T)

    companion object {
        suspend fun <T> loading(data: T, percents: Int?): Loading<T> {
            if (percents != null && (percents < 0 || percents > 100)) throw IllegalStateException("percents cant be $percents")
            return Resource.Loading(data = data, percents = percents)
        }
